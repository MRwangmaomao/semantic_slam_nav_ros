...
// 1. 创建字典 mpVocabulary = new ORBVocabulary()；并从文件中载入字典=========================
    ...
    if (has_suffix(strVocFile, ".txt")) //查找是否存在.txt后缀文件
...
// 4. 创建地图显示帧显示 两个显示窗口  Create Drawers. These are used by the Viewer
    ...
    // mpRunDetect->GetDrawFramePointer(mpFrameDrawer);
...
{
    std::string filepath = mFolderPath + filename;
    cout << endl << "保存相机位姿轨迹到文件 " << filepath << " ..." << endl;
    // 单目相机
    if(mSensor==MONOCULAR)
    {
...
{
    std::string filepath = mFolderPath + filename;
    cout << endl << "保存关键帧轨迹 Saving keyframe trajectory to " << filepath.c_str() << " ..." << endl;
...
{
    std::string filepath = mFolderPath + filename;
    cout << endl << "保持相机位姿  " << filepath << " ..." << endl;
    if(mSensor==MONOCULAR)
    {
...
bool System::SaveMap(const string &filename) {
    std::string filepath = mFolderPath + filename;
    cerr << "System Saving to " << filepath << endl;
    return mpMap->Save(filepath.c_str());
}

// 保存八叉树地图=====
void System::SaveOctomap(const char *filename) {
    std::string filepath = mFolderPath + filename; 
    mpViewer->SaveOctomap(filepath.c_str());
}
